...
class ServerConnection(Connection):
    ...
    def connect(self, server, port, nickname, password=None, username=None, ircname=None, connect_factory=connection.Factory()):
        ...
        * connect_factory - A callable that takes the server address and
        ...
        """
        log.verbose("[jaraco.irc] connect(server=%r, port=%r, nickname=%r, ...)", server, port, nickname)
    ...
    def process_data(self):
        ...
        for line in self.buffer:
            log.verbose2("[jaraco.irc] FROM SERVER: %s", line)
            if not line:
                continue
        ...
        if command in ["privmsg", "notice"]:
            ...
            for m in messages:
                if isinstance(m, tuple):
                    ...
                    m = list(m)
                    log.verbose2("[jaraco.irc] command: %s, source: %s, target: %s, arguments: %s, tags: %s",
    ...
    def send_raw(self, string):
        ...
        try:
            sender(bytes)
            log.verbose2("[jaraco.irc] TO SERVER: %s", string)
        except socket.error:
            # Ouch!
            self.disconnect("Connection reset by peer.")
...
class DCCConnection(Connection):
    ...
    def process_data(self):
        ...
        if self.passive and not self.connected:
            ...
            self.socket = conn
            self.connected = 1
            log.verbose2("[jaraco.irc] DCC connection from %s:%d", self.peeraddress,
                self.peerport)
            self.reactor._handle_event(
                self,
        ...
        if self.dcctype == "chat":
            ...
            if len(self.buffer) > 2 ** 14:
                ...
                log.warning("[jaraco.irc] received >16k from a peer without a newline: disconnecting...")
                self.disconnect()
                return
        else:
            chunks = [new_data]
        ...
        target = None
        for chunk in chunks:
            log.verbose2("[jaraco.irc] FROM PEER: %s", chunk)
            arguments = [chunk]
            log.verbose2("[jaraco.irc] command: %s, source: %s, target: %s, arguments: %s", command, prefix, target, arguments)
    ...
    def send_bytes(self, bytes):
        ...
        try:
            ...
            log.verbose2("[jaraco.irc] TO PEER: %r\n", bytes)
        except socket.error:
            self.disconnect("Connection reset by peer.")
...
class SimpleIRCClient(object):
    ...
    def _dispatcher(self, connection, event):
        ...
        """
        log.verbose2("[jaraco.irc] _dispatcher: %s", event.type)
