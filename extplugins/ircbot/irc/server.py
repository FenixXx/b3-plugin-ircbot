...
class IRCClient(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        log.verbose('[jaraco.irc] client connected: %s', self.client_ident())
    ...
    def _handle_line(self, line):
        try:
            log.verbose2('[jaraco.irc] from %s: %s' % (self.client_ident(), line))
            command, sep, params = line.partition(' ')
            handler = getattr(self, 'handle_%s' % command.lower(), None)
            if not handler:
                log.verbose2('[jaraco.irc] no handler for command: %s. Full line: %s', command, line)
                raise IRCError.from_name('unknowncommand', '%s :Unknown command' % command)
            response = handler(params)
        except AttributeError as e:
            log.error('[jaraco.irc] %s', six.text_type(e))
            raise
        except IRCError as e:
            log.error('[jaraco.irc]  :%s %s %s', self.server.servername, e.code, e.value)
        except Exception as e:
            log.error('[jaraco.irc] :%s ERROR %r', self.server.servername, e)
            raise
    ...
    def _send(self, msg):
        log.debug('[jaraco.irc] to %s: %s', self.client_ident(), msg)
    ...
    def client_ident(self):
        ...
        """
        return NickMask.from_params(self.nick, self.user, self.server.servername)
    ...
    def finish(self):
        ...
        """
        log.verbose('[jaraco.irc] client disconnected: %s', self.client_ident())
        ...
        if self.nick:
            self.server.clients.pop(self.nick)
        log.verbose('[jaraco.irc] connection finished: %s', self.client_ident())
...
def get_args():
    ...
    parser.add_argument("-a", "--address", dest="listen_address", default='127.0.0.1', help="IP on which to listen")
...
def main():
    options = get_args()
    log.verbose("[jaraco.irc] starting irc.server")

    try:
        bind_address = options.listen_address, options.listen_port
        ircserver = IRCServer(bind_address, IRCClient)
        log.verbose('[jaraco.irc] listening on {listen_address}:{listen_port}'.format(**vars(options)))
        ircserver.serve_forever()
    except socket.error as e:
        log.error('[jaraco.irc] %r', repr(e))
        raise SystemExit(-2)
